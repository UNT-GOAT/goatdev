...
OUTPUTS - front_calibration.json for average cm to pixels conversion based on multi-point selection in test images
"""
...
class FrontCalibrationTool:
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if len(self.points) == 1:
                cv2.putText(self.current_image, "Left Edge", (x + 15, y - 10),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
            elif len(self.points) == 2:
                cv2.putText(self.current_image, "Right Edge", (x + 15, y - 10),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    ...
    def calibrate_image(self, image_path: str):
        ...
        instructions = [
            "Click on the LEFT edge of the floor grating",
    ...
    def save_calibration(self, output_file: str = 'front_calibration.json'):
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python calibration_tool_front.py image1.jpg image2.jpg ...")
        print("   Or: python calibration_tool_front.py images/*.jpg")
        sys.exit(1)
    
    image_paths = sys.argv[1:]
    
    print(f"{'='*60}")
    print(f"Front View Calibration Tool - Floor Grating Width")
    print(f"{'='*60}")
    print(f"Known grating width: {GRATING_WIDTH_CM}cm (16 inches)")
    print(f"Images to process: {len(image_paths)}")
    print(f"\nInstructions:")
    print(f"  1. Click LEFT edge of floor grating")
    print(f"  2. Click RIGHT edge of floor grating")
    print(f"  3. Press SPACE to confirm, ESC to skip")
    print(f"{'='*60}\n")
    
    tool = FrontCalibrationTool()
